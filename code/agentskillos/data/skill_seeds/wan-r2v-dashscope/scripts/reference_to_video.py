...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a short video from a reference image with DashScope Wan R2V.")
    parser.add_argument("prompt", help="Video generation instruction")
    parser.add_argument("--input", "-i", required=True, help="Reference image path or URL")
    parser.add_argument("--output", "-o", default="generated_reference_video.mp4", help="Output video path")
    ...
    payload = {
        ...
        "input": {
            "prompt": args.prompt,
            "media": [{"type": "reference_image", "url": _to_asset_ref(args.input)}],
        },
        "parameters": {
    ...
    for _ in range(120):
        ...
        if task_status in {"FAILED", "CANCELED"}:
            raise RuntimeError(f"r2v_failed:{status_payload}")
        time.sleep(5)
    else:
        raise RuntimeError("r2v_timeout")
