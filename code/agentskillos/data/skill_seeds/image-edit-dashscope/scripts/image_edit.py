...
def main() -> int:
    parser = argparse.ArgumentParser(description="Edit an image with DashScope Wan image editing.")
    ...
    parser.add_argument("--output", "-o", default="edited_image.png", help="Output image path")
            ...
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"image": _to_asset_ref(args.input)},
                        {"text": args.prompt},
                    ],
                }
            ]
    ...
    for _ in range(90):
        ...
        if task_status in {"FAILED", "CANCELED"}:
            raise RuntimeError(f"image_edit_failed:{status_payload}")
        time.sleep(2)
    else:
        raise RuntimeError("image_edit_timeout")
