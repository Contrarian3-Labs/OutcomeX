...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a short video from a text prompt with DashScope Wan T2V.")
    ...
    for _ in range(120):
        ...
        if task_status in {"FAILED", "CANCELED"}:
            raise RuntimeError(f"t2v_failed:{status_payload}")
        time.sleep(5)
    else:
        raise RuntimeError("t2v_timeout")
