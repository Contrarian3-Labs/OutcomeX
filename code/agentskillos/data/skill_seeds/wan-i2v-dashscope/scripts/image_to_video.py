...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a short video from an image with DashScope Wan I2V.")
    ...
    parser.add_argument("--output", "-o", default="generated_video.mp4", help="Output video path")
    ...
    payload = {
        ...
        "input": {
            "prompt": args.prompt,
            "media": [{"type": "first_frame", "url": _to_asset_ref(args.input)}],
        },
        "parameters": {
    ...
    for _ in range(120):
        ...
        if task_status in {"FAILED", "CANCELED"}:
            raise RuntimeError(f"i2v_failed:{status_payload}")
        time.sleep(5)
    else:
        raise RuntimeError("i2v_timeout")
