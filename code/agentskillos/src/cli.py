...
from constants import SKILL_GROUPS, SKILL_GROUP_ALIASES, resolve_skill_group
...
def cmd_build(args):
    ...
    if group:
        resolved_runtime_group = resolve_skill_group(resolved_group)
        skills_dir = resolved_runtime_group["skills_dir"]
        output_path = resolved_runtime_group["tree_path"]
        vector_db_path = resolved_runtime_group.get("vector_db_path")
...
def cmd_update_layer(args):
    ...
    resolved_runtime_group = resolve_skill_group(resolved_group)
    tree_path = Path(resolved_runtime_group["tree_path"])
