...
def resolve_skill_group(group_id: str) -> dict:
    """Resolve a skill group ID and patch missing local paths with a safe fallback.

    The repo may declare larger skill groups in config even when a local checkout
    only contains `skill_seeds`. Runtime code should keep working in that case.
    """
    resolved = SKILL_GROUP_ALIASES.get(group_id, group_id)
    requested = next((g for g in SKILL_GROUPS if g["id"] == resolved), None) or SKILL_GROUPS[0]
    fallback = next((g for g in SKILL_GROUPS if g.get("is_default")), SKILL_GROUPS[0])

    materialized = dict(requested)
    patched_fields: dict[str, str] = {}
    for key in ("skills_dir", "tree_path", "vector_db_path"):
        current = materialized.get(key)
        if _path_exists(current):
            continue
        fallback_value = fallback.get(key)
        if _path_exists(fallback_value):
            materialized[key] = fallback_value
            patched_fields[key] = str(fallback_value)

    if patched_fields:
        materialized["fallback_group_id"] = fallback["id"]
        materialized["patched_paths"] = patched_fields
