...
def test_invalid_or_missing_session_credentials_are_rejected(client: TestClient) -> None:
    ...
    assert invalid_list.headers["cache-control"] == "no-store, private"
    assert "X-Attachment-Session-Token" in invalid_list.headers["vary"]
...
def test_expired_session_is_rejected_and_cleaned_up(client: TestClient) -> None:
    ...
    upload_response = client.post(
        ...
        data={"session_id": session["session_id"]},
        headers={"X-Attachment-Session-Token": session["session_token"]},
        files={"file": ("brief.txt", b"payload", "text/plain")},
    )
    assert upload_response.status_code == 201

    with get_container().session_factory() as db:
        attachment_session = db.get(AttachmentSession, session["session_id"])
        assert attachment_session is not None
        attachment_session.expires_at = utc_now() - timedelta(minutes=1)
    ...
    assert expired_list.status_code == 401
    assert expired_list.headers["cache-control"] == "no-store, private"
    assert "X-Attachment-Session-Token" in expired_list.headers["vary"]

    with get_container().session_factory() as db:
        assert db.get(AttachmentSession, session["session_id"]) is None
        remaining = db.query(Attachment).filter(Attachment.attachment_session_id == session["session_id"]).count()
        assert remaining == 0


def test_per_session_attachment_count_quota_is_enforced(
    monkeypatch: pytest.MonkeyPatch, client: TestClient
) -> None:
    monkeypatch.setattr(attachments_service, "MAX_ATTACHMENTS_PER_SESSION", 1)
    monkeypatch.setattr(attachments_service, "MAX_TOTAL_BYTES_PER_SESSION", 10 * 1024 * 1024)
    session = _issue_session(client)
...
def test_metadata_validation_rejects_overlong_filename_and_content_type(client: TestClient) -> None:
    ...
    long_filename = f"{'a' * 513}.txt"
    filename_response = client.post(
        "/api/v1/attachments",
        data={"session_id": session["session_id"]},
        headers={"X-Attachment-Session-Token": session["session_token"]},
        files={"file": (long_filename, b"payload", "text/plain")},
    )
    assert filename_response.status_code == 422
    assert "filename exceeds" in filename_response.json()["detail"]
    ...
    content_type_response = client.post(
        ...
        data={"session_id": session["session_id"]},
        headers={"X-Attachment-Session-Token": session["session_token"]},
        files={"file": ("brief.txt", b"payload", long_content_type)},
    )
    assert content_type_response.status_code == 422
    assert "content_type exceeds" in content_type_response.json()["detail"]


def test_download_not_found_includes_private_cache_headers(client: TestClient) -> None:
    session = _issue_session(client)
    response = client.get(
        "/api/v1/attachments/not-found/download",
        params={"session_id": session["session_id"]},
        headers={"X-Attachment-Session-Token": session["session_token"]},
    )
    assert response.status_code == 404
    assert response.headers["cache-control"] == "no-store, private"
    assert "X-Attachment-Session-Token" in response.headers["vary"]
