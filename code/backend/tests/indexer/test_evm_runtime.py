...
def test_build_subscriptions_reads_onchain_address_environment_variables(monkeypatch) -> None:
    ...
    monkeypatch.setenv("OUTCOMEX_ONCHAIN_ORDER_PAYMENT_ROUTER_ADDRESS", "0x0000000000000000000000000000000000000134")
    ...
    assert any(item.contract_name == "OrderPaymentRouter" and item.event_name == "PaymentFinalized" for item in subscriptions)
